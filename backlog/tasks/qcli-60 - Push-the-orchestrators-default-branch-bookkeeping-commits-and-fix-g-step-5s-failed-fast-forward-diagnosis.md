...
FIX PASS 2 (mandatory review, attempt 2/2): reviewer confirmed all nine ACs and confirmed fix-pass-1's own ancestor-check-primary/path-diff-fallback design against real history (e532f22 vs PR #69's 6008456 touching the same task file). Two BLOCKING findings, both self-contradictions in fix-pass-1's own new prose, plus one LOW taken.

CONTRADICTION (reference/wave-loop.md): the recovery block contradicted itself at its own decision point. Old :209 ended step 2 with "Any commit failing the class check, or failing both the ancestor check and the content-diff fallback -- content this recovery cannot explain or cannot confirm survives -- STOP and escalate ... do not reset past it." Old :212 (step 3) said of the identical input state -- class check passed, ancestor/diff check failed -- that the commit is "not safe to discard ... Instead: reset --hard ... cherry-pick ... push." Same input (class-pass, content-unproven), opposite instructions. The mid-wave in-flight-pointer commit P -- the exact case the whole fix exists to protect -- matches :209's own "passed class, cannot confirm content survives" wording, so an orchestrator obeying :209's STOP never reaches :212's preserve path, defeating the fix. Compounding problem: :202 gates the fallback as running "instead of" the ancestor check (only one of the two ever runs for a given commit), so old :209's "failing both" was literally unreachable -- its only operative half was the "or cannot confirm survives" framing, which named :212's preserve case as an escalation trigger. reference/escalation.md:38 mirrored the identical contradiction in its escalate clause ("A commit failing the class check, or failing both content checks -> escalate, never guess"), one clause after correctly stating the class-pass/content-fail -> cherry-pick rule.

CORRECTED SINGLE RULE (now stated identically in both files): class-check failure -> escalate (content this recovery cannot explain at all); class pass + content unproven (ancestor check false, or its fallback non-empty) -> step 3's preserve case (reset + cherry-pick + push), not an escalation; class pass + content proven -> discard (reset --hard alone).

wave-loop.md:209 now reads: "Any commit failing the class check -- content this recovery cannot explain at all -- STOP and escalate per `reference/escalation.md`'s error-handling table; do not guess and do not reset past it. A commit that passes the class check but whose content presence cannot be proven -- the ancestor check returns false, or (when it runs instead) the content-diff fallback is non-empty -- is not an escalation: it is step 3's preserve case."

escalation.md:38's escalate clause now reads: "...Then resume at step 6. A commit failing the class check -> escalate, never guess" (dropped "or failing both content checks").

RE-TRACE of mid-wave P through the edited text: wave members A, B; P (campaign doc only) sits on already-pushed dispatch-marking commit M, P's own push fails, walk proceeds. A's branch (re-pinned onto M before P existed) rebases clean, squashes to S_A (parented on M, never touching the campaign doc). (g) step 5 fails: local-only=[P]. Step 2: class check passes (P is this wave's own unpushed in-flight-pointer commit). Content check: `git merge-base --is-ancestor P <branch-A>` is false (P postdates every worktree's re-pin, matching the text's own "never an ancestor of any worker branch" case); branch A still exists so the diff fallback never runs. Per corrected :209, class-pass + ancestor-false is explicitly "not an escalation: it is step 3's preserve case" -- no STOP is hit. Step 3: reset --hard origin/<default> (lands on S_A), `git cherry-pick P` applies cleanly (S_A never touched the campaign doc, so it is not empty) -- P's content survives as a new SHA on top of S_A, then pushed. Confirms the fix: P now reaches the preserve path without ever hitting a STOP.

FIX 3 (LOW, taken): two related overclaims closed. (a) wave-loop.md:202 said the dispatch-marking commit is "always an ancestor of `<branch>`" -- not quite: if origin/<default> moves between (d) step 4's push and (g)'s own step-2 rebase (e.g. a concurrent human push), that rebase replays the marking commit under a new SHA, so the *local* copy is no longer literally an ancestor and the check returns false for content that did reach the remote. Softened to "normally an ancestor ... unless [that rebase] replayed it under a new SHA," with the mechanism spelled out. (b) Added to step 3 (:212): an *empty* cherry-pick ("The previous cherry-pick is now empty") is not a conflict and not an escalation -- it means the replayed-SHA case applied, the content is already in `origin/<default>` under the new SHA; treat it as the discard case (`git cherry-pick --skip`, or `--allow-empty` only if the wave log needs the marker commit itself preserved) and continue. Fail-safe either way (halts, never silently loses content), but closes a genuine dead end where the unfixed text routed to a cherry-pick that could not succeed as written.
