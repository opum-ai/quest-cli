...
3. SKILL.md R2: add a new step after the existing 4-item list (as step 5) stating that in-review/merge-pending will never be present in committed state, so R2 must not cross-check leftovers against them; instead derive review substage from: the orchestrator's own <default> checkout dirtiness (step 1) with the specific label present pinning pre/post-approve, the worktree's own git log (step 2) for the worker's stage-4 commit, and gh pr list (step 3) since a PR opens only post-approve. State the fallback: no such signal -> presumptively pre-approval, resume at review.
