...
| Failed fast-forward at (g) step 5 (`git pull --ff-only origin <default>` rejected — "Diverging branches can't be fast-forwarded") | Not the clean-checkout precondition (already reconfirmed at step 0) and not a routine conflict — a bug in the loop (`QCLI-60`). STOP the walk for this member. Run `reference/wave-loop.md` section (g)'s recovery: for each local-only commit, confirm it is this wave's own unpushed dispatch-marking/in-flight-pointer commit (d step 4; i's Scope note) or predates `QCLI-60`, **and** prove its content actually reached `origin/<default>` — ancestor of the just-squash-merged branch, or an empty content diff — before treating it as safe; class membership alone does not prove that, and a mid-wave/R3-recovery in-flight-pointer commit typically passes the class check but fails the content check. Only a commit passing both may be discarded via `git reset --hard origin/<default>`; a commit passing the class check but failing the content check must instead be reset-then-cherry-picked back on and re-pushed, never discarded. Then resume at step 6. A commit failing the class check, or failing both content checks → escalate, never guess |
