...
| Failed fast-forward at (g) step 5 (`git pull --ff-only origin <default>` rejected — "Diverging branches can't be fast-forwarded") | Not the clean-checkout precondition (already reconfirmed at step 0) and not a routine conflict — a bug in the loop (`QCLI-60`). STOP the walk for this member. Run `reference/wave-loop.md` section (g)'s recovery: prove no content was lost — diff local-only commits against `origin/<default>` and confirm each is this wave's own unpushed dispatch-marking/in-flight-pointer commit (d step 4; i's Scope note), or predates `QCLI-60` — before any history-discarding command; only then `git reset --hard origin/<default>` and resume at step 6. An unaccounted-for local-only commit → escalate, never guess |
