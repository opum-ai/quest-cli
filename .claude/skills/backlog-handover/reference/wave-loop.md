...
4. Leave it **uncommitted** across both edits, and discard the accumulated diff (`git checkout -- <path>`) once, before that task's branch reaches (g)'s rebase step — this is what keeps (g)'s clean-checkout precondition true.
