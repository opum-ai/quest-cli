...
**Deliberate divergence from upstream (`QCLI-43`; see SKILL.md Provenance).** `docs/log.md` drifted from `<default>` after every campaign before this rule existed — QCLI-35 closed it for doc-7, QCLI-39 closed it for doc-8, doc-9 reopened it again — but measurement shows two distinct failure modes, not one, and only the first is the squash-merge trap the task description originally blamed for every reopening.

1. **Squash-rewrite / dangling SHAs.** QCLI-32's sync ran as a one-shot on its own per-task branch; that branch's squash-merge (#48) then rewrote its own commit SHA, so the entry the sync had recorded was never an ancestor of `<default>` — four SHAs went dangling, measured at `2e57876` (`6f4dc18b…`, `a357d12f…`, `a6fc2021…`, `fcb814f4…`). This mode fired **once**. QCLI-35's fix closed it for good: 0 bad of 86 measured immediately after QCLI-35's merge (`2b30560`), and it never recurred — 0 bad of 92 immediately after QCLI-39's merge (`1f252dd`) too.
2. **Staleness.** What actually reopened the task after both QCLI-35 and QCLI-39 was ordinary staleness — new `docs/`-touching commits landing on `<default>` with nothing running the sync to catch them up. Missing entries, not corrupted ones. This is the mode that has driven every reopening since QCLI-35, and it is the dominant one.

The remedy is unaffected by getting the history right. Running the sync from settlement, on `<default>`, strictly after each wave's own merges (g) land, fixes staleness by construction — it can no longer fail to run, because it is now part of every wave's settlement instead of a task someone has to remember to file — and it keeps the dangling-SHA mode closed for the same structural reason it closed after QCLI-35: settlement runs "orchestrator only, on `<default>` directly. Never on a per-task branch" (above), so every SHA the sync ever records is already, permanently, part of `<default>`'s own history at record time — never a branch commit awaiting a squash-merge that could invalidate it. A commit already on `<default>` stays an ancestor of `<default>` forever under this skill's merge model (fast-forward only; no rebase or force-push of `<default>` anywhere in this skill — see g, R5).
...
2. **Commit the docs changes.** Read the `files` list from the `lore sync --json` call above's JSON payload at `.data.files[].path`, e.g. `jq -r '.data.files[].path'` — a real payload looks like `{"schemaVersion":1,"kind":"sync.result","data":{"files":[{"path":"docs/log.md"}],"filesChanged":1,"backlogCommit":{"committed":false,"files":[]},"dryRun":true,"orphanedIndexes":[]}}`. `git add` exactly those paths and commit them together as one commit, e.g. `docs: sync lore log after wave-<N> settlement (doc-<M> campaign)`, with one `Refs: QCLI-<N>` trailer per task resolved this wave. An empty `files` list means this wave's resolved tasks touched no lore-tracked doc — nothing to commit, not an error.
