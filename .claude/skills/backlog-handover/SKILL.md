...
| **Commits** | Project commit conventions; always a `Refs: QCLI-<N>` trailer — except `lore sync`'s own `backlog/` auto-commit (hardcoded in the lore binary as `chore(backlog): sync task changes`, no trailer; the one deliberate exception — see `reference/wave-loop.md` section i) |
