...
| **Commits** | Project commit conventions. Hybrid trailer rule (`QCLI-47`, refining `QCLI-43`'s single exception): a `Refs: QCLI-<N>` trailer is **required** on every commit that has one directing task — every worker/implementation commit, and every orchestrator bookkeeping commit for a single task (dispatch marking, in-flight pointer recording, settlement). Two exceptions carry **no** trailer, both genuinely task-less: `lore sync`'s own `backlog/` auto-commit (hardcoded in the lore binary as `chore(backlog): sync task changes`, outside this skill's control — `QCLI-43`'s original exception), and campaign-scoped bookkeeping commits with no single directing task — campaign init, campaign close, handover archiving, and the one-time `.claude/handovers/` gitignore setup at I3. Full per-commit-type breakdown in `reference/wave-loop.md` section i |
