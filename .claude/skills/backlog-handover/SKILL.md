...
**Deliberate divergence (`0.9.1-qcli.2`, `QCLI-43`, 2026-08-07):** upstream settlement (R4i) ends at the campaign-doc write. This fork adds a mandatory `lore sync` + `lore check` step to the end of every wave's settlement (`reference/wave-loop.md` section i), so `docs/log.md` and the bundle's managed blocks catch up to `<default>` in the same session that moves it — closing a drift that had reopened three times (QCLI-35, QCLI-39, and again before this task) because the fix always ran as a one-shot sync **on a per-task branch**, which that branch's own squash-merge then invalidated. Upstream has no lore/OKF docs bundle and nothing analogous to drift, so this is not a gap upstream needs closed — it is specific to this repo's docs tooling and is not proposed for promotion upstream. See `QCLI-43` for the full analysis, including why running the sync from settlement (on `<default>`, strictly after that wave's merges land, never on a per-task branch — a constraint this skill already enforced for unrelated reasons) is what makes the recorded SHAs permanent `<default>` ancestors instead of doomed branch commits.
