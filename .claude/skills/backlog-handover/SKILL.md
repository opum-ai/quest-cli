...
| **Stage state** | Backlog has three statuses, the campaign has six stages. Status carries the coarse state; labels carry the sub-stage for four of the six stages — for the other two (`in-review`, `merge-pending`) the label is applied to the working tree and deliberately discarded before it is ever committed (`QCLI-51`), so it carries no sub-stage in Backlog's committed state at all. See the durability column in the state table below for exactly which four vs which two |
...
Disposition: the Stage-state row now states the split inline — labels carry the sub-stage for four of the six stages, not all six, with the other two named and cited to `QCLI-51` — so the qualification is visible before the reader reaches the table, not only inside it. R2 step 5 now names the in-flight table as a fourth signal, but the addition is narrower than doc-12 wave 1's own framing offered ("arguably stronger evidence than any of the three currently listed"): checking the two real in-flight-pointer-recording commits against actual history (`68ce681`, doc-12 wave 1; `82fca71`, doc-13 wave 1 — this campaign's own dispatch of this task) found both record only stage 1 (dispatch), and neither `SKILL.md` nor `reference/wave-loop.md` mandates a further recording pass at every later stage transition. The commit itself is durably never-dirty, but that does not make the table's content continuously current, so the added text treats a later recorded stage as positive proof of progress and its absence as inconclusive, not as proof progress stalled — the original three signals still carry that case. Whether `reference/wave-loop.md` should be tightened to mandate additional recording passes (e.g., at approval) is a distinct question, flagged via `--comment` on `QCLI-52` rather than decided here.
