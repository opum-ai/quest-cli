...
This project uses Quest CLI 0.2.2 for tracker operations. Run \`quest manifest --json\` to discover the supported command contract. Use \`quest instructions --json\` for the current versioned protocol. For Backlog tracker cutover, run \`quest migration backlog preview --source <project> --json\`, review its digest and mappings, then apply it with \`quest migration backlog apply --source <project> --digest <digest> --actor <id> --actor-kind human --json\`. Quest writes require an explicit actor declaration; do not edit Quest-authored records directly.
${end}\n`;
...
export function checkQuestAgentInstructions(
  ...
  if (`${blocks[0]}\n` !== questAgentInstructions) {
    return {
      state: "drift",
      message: "Quest agent instruction block differs from version 0.2.2.",
    };
  }
