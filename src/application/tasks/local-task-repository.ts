  ...
  implements LifecycleTaskRepository, MigrationTransactionRepository
{
  ...
  /**
   * Typed retained-location persistence for the batch session (QCLI-243):
   * the batch's own lock is the same `.write.lock` file `writeLifecycle`
   * acquires, so this is safe to call directly for the duration of an open
   * session without re-deriving `writeLifecycle`'s CAS/journal machinery.
   */
  async writeRetiredRecord(
    task: TaskState,
    location: TaskLocation,
  ): Promise<void> {
    await this.writeLifecycleRecord(this.taskPath(task.id, location), task);
  }
...
export interface TaskBatchSession {
   ...
   * `location` defaults to "tasks" (QCLI-243): a batch item resolved from a
   * completed or archived record must write back to that same location, or
   * a plain active-directory write would leave a stray duplicate in
   * `tasks/` while the real retained record stays untouched.
   */
  writeRecord(task: TaskState, location?: TaskLocation): Promise<void>;
...
class LocalTaskBatchSession implements TaskBatchSession {
  ...
  async writeRecord(
    task: TaskState,
    location: TaskLocation = "tasks",
  ): Promise<void> {
    if (location === "tasks") {
      await this.repository.writeActiveRecord(task);
    } else {
      await this.repository.writeRetiredRecord(task, location);
    }
  }
