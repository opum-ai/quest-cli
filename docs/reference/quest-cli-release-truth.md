...
## Current publication state (2026-09-07, updated for 0.4.0)

- npm dist-tag `latest` = `0.4.0` on the root package and all six platform
  packages (verified independently via `npm view @opum-ai/quest version` and
  `npm view <platform-package> version` for all six, after publication).
- Published via `release.yml` `workflow_dispatch` `publish=true` against tag
  `v0.4.0` (dry run
  https://github.com/opum-ai/quest-cli/actions/runs/34133338726, real publish
  https://github.com/opum-ai/quest-cli/actions/runs/34133947526), gated on a
  native-execution receipt fetched by that same workflow for commit
  `43441cc`. `dev` was promoted to `main` (fast-forward,
  `git push origin dev:main`) before the tag was cut, continuing 0.3.4's
  corrected order.
- The publish run's own post-publish verification step failed after its
  5-minute retry budget because `@opum-ai/quest-win32-arm64` specifically
  stayed in npm's async provenance-attestation "being processed" state
  longer than that window (its publish log shows npm's own "Your package is
  being processed and may take a few minutes to become available" notice,
  immediately after a successfully signed provenance statement was published
  to the Sigstore transparency log). This was diagnosed as npm-side
  propagation lag on one package, not a publish failure: the `Publish` step
  itself succeeded for all seven packages, the other six were visible within
  the retry window, and `quest-win32-arm64` resolved shortly after on
  independent polling -- no republish was attempted or needed.
- Publication authority: the repository's own user explicitly authorized
  this release in-session (2026-09-07, `AskUserQuestion`) to carry QCLI-229
  (`quest task pause`/`quest task start`, and `quest task demote`'s new
  required `--to` argument) to a release tag, after `opum-agent` lifted an
  earlier hold pending `lore` 0.4.6 (verified independently on the registry
  before the ask: `npm view @opum-ai/lore version` -> `0.4.6`).
- See "0.4.0 release" below for the qualification, receipt, and
  post-publish-smoke evidence for this specific version.
...
Carries QCLI-229: a new default-on, non-terminal `Blocked` status reached
only via `quest task pause <id>` and left only via `quest task start <id>`
(the generic `--status` edit can reach neither), and `quest task demote`'s
new required `--to "<status>"` argument (omitting it is now a usage error
with no mutation; an illegal target names the current status and the legal
targets instead of the prior opaque `task_lifecycle_already_at_destination`).
Also carries QCLI-241's own version-bump mechanics, plus everything `dev`
held since 0.3.4 (the fleet operating block v8 join and tracker hygiene).

Source commit `43441cc` (PR #284 merge to `dev`, then promoted to `main`
via PR #285 + `git push origin dev:main` before the tag was cut, matching
0.3.4's corrected order). Prepublication qualification run
https://github.com/opum-ai/quest-cli/actions/runs/34130538518 (explicit
`workflow_dispatch` against the `v0.4.0` tag) passed all six platform jobs
plus `source-gates`, `candidate-bundle`, `native-execution-receipt`, and the
operating-block digest check.

The native-execution receipt binds `0.4.0` at `43441cc` across all six
platforms (`bun run receipt:require` confirmed this locally before publish,
and `bun run receipt:verify-published -- 0.4.0` confirmed it again against
the published registry afterward).

npm `dist.shasum`/`dist.integrity` observed live after publication, for the
root package and all six platform packages:

| Package | npm shasum | npm integrity |
| --- | --- | --- |
| `@opum-ai/quest@0.4.0` | `0bc834c75ac644ce0c7e1b87b85a621fa726280d` | `sha512-nQy9U4dSoFV1TNoLCm5pJ5S++4SaSWILKGD6Zymm90XYzx/Rzgz5J0pkkD/38wKBd906lZcuZgYmkCmXP/KNLg==` |
| `@opum-ai/quest-darwin-arm64@0.4.0` | `c3151cdd8d4f56ed9f7d7aba634c355cb80e48cb` | `sha512-ouWzLG48hfczXw2h0KP8ya+w4D93bpjQHhSC3QkFBBC1pfcOszp/cwWdq0TYVym1vNfcGrzPZhsTV0G3O/d7Qw==` |
| `@opum-ai/quest-darwin-x64@0.4.0` | `ce21e6df2e68a55711f54a9bc854d6672f7a995a` | `sha512-o1KL+xy6uYRdV1f5mtHRv3D8RgTQeLImV3CFB+GSxaoY/IQKn3QXW/fBP2lbfcj8YIDxZ6MIMNdxWaFgP/35cA==` |
| `@opum-ai/quest-linux-arm64@0.4.0` | `d260b2e7ab5e714a3910245b049764bac0beb835` | `sha512-MtyV5+T0okjx4glorr+YoA7lpVJTPgdkYjN+IPtboh5A9SQg3Cr97N3akzeG6iOljXvcbansbID65tW/IEw4Ng==` |
| `@opum-ai/quest-linux-x64@0.4.0` | `f5f003278e6e274b59291d3227e0fd2f675e7ae5` | `sha512-OCrViJN/jPHhHl2b/SrGRQnoXBWuXyVGsp/yjqWKYWh90g1ZRQsWF19R0VXQ7RYGQM5JvlV4emS8KYxqVr8n7Q==` |
| `@opum-ai/quest-win32-arm64@0.4.0` | `aabfd6c5f74b3c39525d86b6f3d20ca3c8f6ee64` | `sha512-8/pw0uezDMSi2yVLdyL61YhbhM3cN42WYnVnCq8rAqfgL/m1oqwYYlrcnStBDoOaRTmOMyqIumhdzV1PwjMqYA==` |
| `@opum-ai/quest-win32-x64@0.4.0` | `ca682dbce67f11ed896cf6fa00634f3ca6f61335` | `sha512-pAjKB/xcP6WvFXQRk8EXWHbenkQe+CRLc1CfVOikTeYhvNuYrJprAT+Zn1011PyfdJwPqP2zLhx+FKexviHE1A==` |

Post-publish clean-install smoke (a fresh `npm install @opum-ai/quest@0.4.0`
in an isolated scratch directory, no local checkout involved) proved the new
lifecycle commands end to end against the published bytes: `quest --version`
reports `0.4.0`; `quest manifest --json` lists both `task pause` and
`task start`, with `task demote` carrying `filters:["to"]`; a full
`init` -> `task create` -> `task edit --status "In Progress"` ->
`task pause` (-> `Blocked`) -> `task start` (-> `In Progress`) ->
`task demote --to "To Do"` (-> `To Do`) sequence completed exactly as
designed.
